...
import { Heart, MessageCircle, Play, Edit3, Trash2, Eye, EyeOff, X } from 'lucide-react';
            ...
            <div className="relative aspect-video overflow-hidden shrink-0">
                ...
                {isOwner && onEdit && onDelete && (
                    <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                        {onTogglePublish && (
                            <button onClick={(e) => { e.stopPropagation(); onTogglePublish(video._id); }} className={`p-3 bg-white/90 dark:bg-black/60 backdrop-blur-md rounded-2xl shadow-xl hover:text-white transition-all active:scale-90 ${video.isPublished ? "text-green-600 hover:bg-green-600" : "text-gray-600 dark:text-gray-400 hover:bg-gray-600"}`} title={video.isPublished ? "Unpublish" : "Publish"}>
                                {video.isPublished ? <EyeOff size={18} /> : <Eye size={18} />}
                            </button>
                        )}
                        <button onClick={(e) => { e.stopPropagation(); onEdit(video); }} className="p-3 bg-white/90 dark:bg-black/60 backdrop-blur-md text-blue-600 dark:text-blue-400 rounded-2xl shadow-xl hover:bg-blue-600 hover:text-white transition-all active:scale-90"><Edit3 size={18} /></button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(video._id); }} className="p-3 bg-white/90 dark:bg-black/60 backdrop-blur-md text-red-600 dark:text-red-400 rounded-2xl shadow-xl hover:bg-red-600 hover:text-white transition-all active:scale-90"><Trash2 size={18} /></button>
                    </div>
                )}
                ...
                {onRemoveFromHistory && (
                    <button
                        ...
                        className="absolute top-3 right-3 p-2 bg-black/60 hover:bg-red-600 text-white rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all z-20 active:scale-90"
                        ...
                        <img
                            ...
                            onError={(e) => {
                                ...
                                if (!target.src.includes("ui-avatars.com")) {
                                    target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(video.owner?.username || 'User')}&background=random`;
                                }
                            }}
                        />
                    </Link>
